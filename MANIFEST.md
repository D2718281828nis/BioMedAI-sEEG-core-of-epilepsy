...
| `EdfRunResult` | [`models.py`](src/extreme_event_agent/models.py) | The typed result of `run_edf`: report, process, the montage dict and which reference was actually analysed (`montage_reference`), the montage file path, every figure path (including the layout dict and message-passing pair), the GraphML export path, the message-passing evaluation dict, and both event-resolution fallbacks. |
